format longg
%% Task 1C
% Determine wheter measurement update is enabled
measurement = 0;
% Constants
...
for i = 1:length(t)
    ...
    % If measurement update is enabled
    if measurement == 1
    ...
    % If measurement update is disabled
    else
        x_u_est = x_p_est;
        P_u_est = P_p_est;
    end
    ...
    x_est = x_u_est;
    P_est = P_u_est;
    % Uncertainty of the x-direction position and velocity
    u_p_x(i) = sqrt(P_est(1,1));
    u_v_x(i) = sqrt(P_est(4,1));
end
% disp(x_NED)

% When measurement update is enabled, u_p_x and u_v_x converges to 
% 2.14676705715485 and 1.69256585060291 respectively.
% When measurement update is disabled, the uncertainties accumulates over
% time.